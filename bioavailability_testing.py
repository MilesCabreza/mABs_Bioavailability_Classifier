...
csv = 'New_sequences_clustered.csv'
df = pd.read_csv(csv)
heavy_seqs = df['Heavy']
light_seqs = df['Light']
labels = df['High/Low Bioavailability']
...
class Classifier:
    ...
    def create_inputs_antiBERTy(self, heavy_seqs, light_seqs, labels, clusters):
    ...
    def create_inputs_ABlang(self, heavy_seqs, light_seqs, labels, clusters):
    ...
    def create_inputs_ESM(self, heavy_seqs, light_seqs, labels, clusters):
    ...
    def tune_hyperparameters_logo(self, X, y, groups):
    # tunes the parameters of selected model using leave one group out validation gridsearch
        logo = LeaveOneGroupOut()
        self.grid_search = GridSearchCV(self.pipeline, self.param_grid, cv=logo)
    ...
    def logo_validate(self, X, y, groups):
        if not self.best_clf:
            raise ValueError('Model not tuned. Call tune_hyperparameters() first.')
        logo = LeaveOneGroupOut()

        scores = cross_validate(self.best_clf, X, y, cv=logo, groups=groups, return_train_score=True)
        ...
        logo_df = pd.DataFrame({
            'Train Scores (All Groups)': [rounded_train_scores],  # Store scores as lists
            'Test Scores (All Groups)': [rounded_test_scores],
    ...
    def alldata_test(self, X, y, groups):
        
        def save_logo_models(X, y, groups): 
            
            from os import makedirs
            makedirs("logo_models", exist_ok=True)
            ...
            # Loop through each group/fold in LOGO
            for i, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
                ...
                model_filename = f'logo_models/model_group_{i}.pkl'
    ...
    def run_clf(self, heavy_seqs, light_seqs, labels, clusters, feature_model = None, clf_model = None, save_model = None):
    ...
    # LOGO and training data scores
        ...
        if feature_model == 'antiBERTy':
            X, y, groups = self.create_inputs_antiBERTy(heavy_seqs, light_seqs, labels, clusters)
        elif feature_model == 'ABlang':
            X, y, groups = self.create_inputs_ABlang(heavy_seqs, light_seqs, labels, clusters)
        elif feature_model == 'ESM':
            X, y, groups = self.create_inputs_ESM(heavy_seqs, light_seqs, labels, clusters)
        else:
            print(f'==============================================')
            print("This model does not yet exist within this class")
            print(f'==============================================')

        # preprocessing of features, not needed for SVC/NuSVC model
        selector = SelectKBest(score_func=f_classif, k=50) #selects top 50
        X = selector.fit_transform(X, y)
        pca = PCA(5)  # projects to 5 dimensions
        ...
        if save_model:
            self.tune_hyperparameters_logo(X, y, groups)
            self.alldata_test(X, y, groups) # comment out if you want to test just the base model
            ...
            joblib.dump(saved_pipeline, "SGDClassifier.joblib")
            ...
            # tuning and validation of model for LOGO validation and train scores
            self.tune_hyperparameters_logo(X, y, groups)
            final_printout = self.logo_validate(X, y, groups)
...
# df = bioavail_classifier.run_clf(heavy_seqs, light_seqs, labels, clusters, 'antiBERTy', 'SGDClassifier', True)
# print(df)
# df.to_excel('Bioavailability_Classifier_Stats.xlsx', index=False, sheet_name='Test_Results')

"""**5. Running top 5 algorithms for each feature generating model**"""

def run_top_5_classifiers(heavy_seqs, light_seqs, labels, clusters):
    ...
    # for feature_model in ["antiBERTy", "ABlang", "ESM", "DeepSP"]:
    for feature_model in ["ESM"]:
        for clf in top_5_classifiers:
            # Run the classifier and get results (assume clf_results_df is modified accordingly)
            clf_results_df = bioavail_classifier.run_clf(heavy_seqs, light_seqs, labels, clusters, feature_model, clf)
...
df = run_top_5_classifiers(heavy_seqs, light_seqs, labels, clusters)
df.to_excel("Bioavailability_Classifier_Stats.xlsx", index=False, sheet_name="testing")
print(df)
